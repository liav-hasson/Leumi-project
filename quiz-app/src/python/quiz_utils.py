    ...
    if isinstance(DATABASE, list):
        ...
        for item in DATABASE:
            if item.get('name') == category:
                ...
                elif 'subjects' in item and isinstance(item['subjects'], list):
                    # Handle case where subjects is a list
                    return [subject.get('name') for subject in item['subjects'] if 'name' in subject]
        return []
    else:
...
if __name__ == '__main__':
    ...
    if categories:
        ...
        if subjects:
            ...
            print(f"Random keyword for '{categories[0]}' -> '{subjects[0]}':", keyword)
